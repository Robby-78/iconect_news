                ...
                <div class="row">
                    <div class="columns" id="amplitude-left">
                        <div class="amplitude-inner-left">
                            <?php $imgBaseURL = getBaseURLByStorage($post->image_storage); ?>
                            <img src="<?= $imgBaseURL . $post->image_slider; ?>" alt="<?= esc($post->title); ?>"/>
                        </div>
                                            ...
                                            <div class="download-link-cnt">
                                                <?php if ($generalSettings->audio_download_button == 1 && $audio->download_button == 1): ?>
                                                    <form action="<?= base_url('download-file'); ?>" method="post">
                                                        <?= csrf_field(); ?>
                                                        <input type="hidden" name="path" value="<?= esc($audio->audio_path); ?>">
                                                        <input type="hidden" name="name" value="<?= esc($audio->audio_name); ?>">
                                                        <input type="hidden" name="storage" value="<?= esc($audio->storage); ?>">
    ...
    Amplitude.init({
        "songs": [
            ...
            $audioBaseURL = getBaseURLByStorage($audio->storage); ?>
            {
                ...
                'cover_art_url': "<?= $imgBaseURL . $post->image_default;  ?>",
            },
            <?php endforeach; ?>
        ]
    });
